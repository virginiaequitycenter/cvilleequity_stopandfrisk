...
```{r}
totals <- sf %>% filter(type == "STOP WITH SEARCH OR FRISK") %>% 
  group_by(year) %>% 
  summarize(sfyes = n()) %>% 
  left_join(sf %>% filter(type != "STOP WITH SEARCH OR FRISK") %>% 
              group_by(year) %>% 
              summarize(sfno = n())
            ) %>% 
  mutate(total = sfyes+sfno,
         percentyes = round((sfyes/total)*100, 1),
         percentno = round((sfno/total)*100, 1))


ggplot(sf, aes(x = year, fill = type)) + 
  geom_bar() +
  scale_fill_manual(values = pal2, name = "Search/Frisk", labels = c("No", "Yes")) +
  ...
  geom_text(aes(year, sfyes, label = percentyes, fill = NULL, vjust = 1.5), size = 3, data = totals) +
  geom_text(aes(year, total, label = percentno, fill = NULL, vjust = 1.25), size = 3, data = totals) +
