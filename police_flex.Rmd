...
Navigate to additional pages for more further analysis:
...
valueBox(bl_pc, icon = "fa-balance-scale-left")
```
...
valueBox(wh_pc, icon = "fa-balance-scale-right")
```
...
sflocationsmap <-
  sflocations %>%
  mutate(RACE = ifelse(RACE == "W", "White Individuals", "Black Individuals"))
...
sf.df <- split(sflocationsmap, sflocationsmap$SFTYPE)
 ...
 l <<- l %>%
          addCircles(
                data = sf.df[["STOP WITH SEARCH OR FRISK"]],
                color =  "red",
                weight = 2,
                radius = 6,
                fillColor = "transparent",
               group = "Highlight Stop and Frisk"
       )

l %>%
  addLayersControl(
    baseGroups =  names(race.beats),
    overlayGroups =  c(names(race.df), "Highlight Stop and Frisk"),
